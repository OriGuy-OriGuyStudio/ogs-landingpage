...
const PromotionBanner = ({ textBottom, textTop }: PromotionBannerProps) => {
  ...
  return (
    <motion.div
      ...
      className="bg-color-brand-whiteYellow-100-light/8 text-color-brand-whiteYellow-100-light fixed top-0 right-0 bottom-0 left-0 flex h-34 flex-col items-center justify-center gap-2 rounded-br-3xl rounded-bl-3xl px-4 text-center drop-shadow-lg backdrop-blur-sm sm:px-20"
    >
      <motion.p
        ref={topText}
      ...
      <motion.p
        ...
        transition={{ duration: 0.75, ease: 'circIn', delay: 2.25 }}
        whileInView={{ opacity: 1, top: '0rem' }}
      >
        {textBottom}
      </motion.p>
